...
# 新增 --frames 參數，預設為 120 (以 2 fps 計算，約等於一分鐘)
parser.add_argument(
    '-f', '--frames', 
    type=int, 
    default=122, 
    help="設定要收集的總幀數。輸入 0 代表無限制一直收集，直到按下 Ctrl+C，例如 2 fps下一分鐘為 120 + 2 （緩衝）(預設值)"
)
...
def camera_process_worker(DEV_PATH):
    while True:
        ...
        # 原始資料就是 1920x1080
        uyvy_frame = raw_array.reshape((video_height, video_width, 2))
        # 將 UYVY 格式(相機硬體偏好)的色彩空間轉換成一般圖片常用的 BGR 格式（軟體偏好）
        ...
        cv2.imwrite(save_path, bgr_frame, [int(cv2.IMWRITE_JPEG_QUALITY), 80])
...
def collect_data():
    ...
    while True:
        ...
        msg = master.recv_match(type=['HIGHRES_IMU', 'GLOBAL_POSITION_INT', 'LOCAL_POSITION_NED', 'ATTITUDE'], blocking=False)
        
        if msg == None:
            continue
        if msg.get_type() == 'HIGHRES_IMU' and not flag_imu:
            ...
            # print(msg)
            flag_imu = True
        elif msg.get_type() == 'GLOBAL_POSITION_INT' and not flag_gps:
            # print(msg.lat)
            ...
            flag_gps = True
        elif msg.get_type() == 'ATTITUDE' and not flag_atd:
            yaw_rad = msg.yaw
            # 2. 轉換為角度 (-180 到 180)
            ...
            flag_atd = True
        # elif msg.get_type() == 'EKF_STATUS_REPORT' and not flag_ekf:
        ...
        #     flag_ekf = True
        elif msg.get_type() == 'LOCAL_POSITION_NED' and not flag_ned: # z 是向下的，所以取負號變成高度
            altitude = -msg.z
            # sensor_data[9] = msg.x
...
# MAVLink 感測器以 40Hz 的高頻率在背景更新 sensor_data
freq = 25000
...
try:
    while rounds < round_limit:
        ...
        while frame_idx < num_frames:
            # 存取影像的時間間隔控制在 30Hz (約 33.3ms)
            # if time.time() - frame_timestamp < 0.033:
            # 存取影像的時間間隔控制在 2Hz (約 500ms)
            if time.time() - frame_timestamp < 0.5:
                continue
            frame_timestamp = time.time()
